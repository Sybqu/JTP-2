...
## Working strat,
1. Checking which services are running in docker using <docker ps> command
2. Locating the main process and switch to hopperoo
3. Checking the uptime-checker container
4. The deployer container (privillieged) is accessbile to us while will we are still in uptime-checker container
5. We completed a docker escaper attack
6. Run the recovery script and all set

    # Conclusion
```
flag: THM{DOCKER_ESCAPE_SUCCCESS}
